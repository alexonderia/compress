...
Compose запускает локальный Ollama, а сервисы подключаются к нему по адресу `http://ollama:11434` внутри общей сети. Используйте переменные окружения в `docker-compose.yml`, чтобы выбрать нужную модель или адаптировать таймауты. По умолчанию все сервисы ожидают модель `qwen2.5:0.5b` на сервисе `ollama`.
...
OLLAMA_MODEL=qwen2.5:0.5b docker compose up -d
```
