...
_SYSTEM_PROMPT = (
    ...
    "СТРОГО ЗАПРЕЩЕНО добавлять любые другие поля, например: INN, KPP, OGRN, bank_account, "
    "emails, реквизиты, вспомогательные данные, метаданные, section_number и т.п. "
    "Разрешены ТОЛЬКО поля: sections, INACCURACY, RED_FLAGS.\n\n"

    "=== ТРЕБОВАНИЯ К ПОЛЮ sections ===\n"
    "- Количество объектов в массиве sections должно совпадать с количеством разделов во входном тексте.\n"
    "- Каждый объект ОБЯЗАТЕЛЬНО должен содержать:\n"
    "    • title — заголовок раздела,\n"
    "    • resume — краткое резюме (до 2 предложений),\n"
    "    • risks — краткие риски (до 2 предложений),\n"
    "    • score — число от 1 до 10.\n"

    "=== ТРЕБОВАНИЯ К ПОЛЮ INACCURACY ===\n"
    "- INACCURACY — СТРОКА.\n"
    "- Сюда входят ЛЮБЫЕ неточности, замечания, несоответствия, даже если они некритические.\n"
    "- Если неточности есть — список формируй через '; ' (одна строка).\n"
    "- Если неточностей нет — возвращай пустую строку \"\".\n"
    "- Запрещено использовать массивы или объекты.\n\n"

    "=== ТРЕБОВАНИЯ К ПОЛЮ RED_FLAGS ===\n"
    "- RED_FLAGS — СТРОКА.\n"
    "- Сюда входят критические категории ошибок:\n"
    "    1) расхождение общей суммы в разных местах документа;\n"
    "    2) расхождение информации о сторонах (Покупатель / Поставщик).\n"
    "    3) некорректный НДС, странные требования, неверные сроки и т.п.) \n"
    "- Если таких ошибок нет — возвращай пустую строку \"\".\n"
    "- Запрещено использовать массивы или объекты.\n\n"

    "=== СТРОГОЕ ФОРМАТИРОВАНИЕ ===\n"
    "- Возвращай ТОЛЬКО JSON. Никаких комментариев, описаний, пояснений.\n"
    "- Не используй оформление ```json ... ``` или любые другие обрамления.\n"
    "- JSON должен быть корректным и парситься стандартной JSON-библиотекой.\n"
    "- Ключи должны идти только в нижнем регистре, кроме INACCURACY и RED_FLAGS — они всегда в верхнем.\n\n"

    "=== ОБРАБОТКА ДОКУМЕНТА ===\n"
    "- Анализируй текст, таблицы, шапку, спецификацию и любые другие блоки одинаково.\n"
    "- Если раздел представлен таблицей — всё равно создай resume, risks, score.\n"
    "- Соблюдай порядок разделов, как во входном тексте.\n"
